...
import com.bi.model.ISearchable;
...
public abstract class SearchServiceImpl<T, R extends QuerydslPredicateExecutor<T> &  PagingAndSortingRepository<T, ?>> implements SearchService<T, R>{
	...
	public Iterable<T> search(String searchText, Class<T> clazz, PaginationConfig paginationConfig) {
		return search(searchText, clazz, paginationConfig, null, null,null);
	}
...
//	    if (log.isDebugEnabled()) {
	...
	/**
	 * Modified for fetching total Count of records present in db (also handles search text)
	 * @param searchText
	 * @param clazz
	 * @param filteredNode
	 * @param booleanAndEx
	 ...
	 */
	@Override
	public Long count(String searchText, Class<T> clazz,List<String> filteredNode,
			BooleanOperation booleanAndEx,BooleanOperation booleanOrEx) {
		Path<T> path = null;
	    ...
	    if(booleanOrEx != null) {
			if(ops != null) {
			    ops = Expressions.predicate(Ops.OR, ops, booleanOrEx);
			} else {
				ops = booleanOrEx;	
			}
		}
	 	...
	 	if(booleanAndEx != null) {
			if(ops != null) {
			    ops = Expressions.predicate(Ops.AND, ops, booleanAndEx);
			} else {
				ops = booleanAndEx;	
			}
		}
	 ...
	 */
	@Override
	public List<T> search(String searchText, Class<T> clazz, PaginationConfig paginationConfig,List<String> filteredNode,
			BooleanOperation booleanAndEx,BooleanOperation booleanOrEx) {
	...
	public List<T> search(String searchText, Class<T> classes[], PaginationConfig paginationConfig,List<String> filteredNode,BooleanOperation booleanEx) {
		...
		if(booleanEx != null) {
			ops = Expressions.predicate(Ops.OR, ops, booleanEx);
			if(ops != null) {
